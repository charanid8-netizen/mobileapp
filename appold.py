...
def main(page: ft.Page):
    ...
    stopwatch_tab = ft.Column([ft.Text("stopwatch", size=20, weight=ft.FontWeight.BOLD),
        stopwatch_text,
        ft.Row([
    ...
    todo_tab = ft.Column([ft.Text("To‑do List", size=20, weight=ft.FontWeight.BOLD),
        todo_input,
        ft.Row([
...
# ---------------- Compact Box Calculator ----------------
    ...
    expand=True,                 # fills the whole tab space
    alignment=ft.Alignment(0,0)  # 👈 pins everything to the middle of the tab
)
    # ---------------- Games ----------------
    catch_score = 0
    time_left = 10
    ...
    catch_game = ft.Column([ ft.Text("Catch Game", size=18, weight=ft.FontWeight.BOLD),ft.Row([catch_text, timer_text], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, width=400), start_btn, catch_game_area],spacing=5, alignment=ft.MainAxisAlignment.START)
    ...
    guessing_game = ft.Column([ft.Text("Guessing Game", size=18, weight=ft.FontWeight.BOLD),guess_input, guess_btn, guess_result],spacing=5, alignment=ft.MainAxisAlignment.START)
    games_tab = ft.Row(
    [
        ft.Container(content=catch_game, expand=1, padding=10),
        ft.VerticalDivider(width=1, thickness=2, color="grey"),   # 👈 NEW line between
        ft.Container(content=guessing_game, expand=2, padding=10)
    ],
    ...
    clock_tab = ft.Container(
    content=ft.Column(
        [
            ft.Text("Clock", size=24, weight=ft.FontWeight.BOLD),
            clock_text
        ],
        spacing=20,
    ...
    expand=True,   # fills available space
    alignment=ft.Alignment(0, 0)
)

    # ---------------- Navigation ----------------
    views = [stopwatch_tab, todo_tab, calc_tab, games_tab, clock_tab]
    ...
    nav = ft.NavigationBar(
        destinations=[
            ...
            ft.NavigationBarDestination(icon=ft.icons.Icons.VIDEOGAME_ASSET, label="Games"),
            ft.NavigationBarDestination(icon=ft.icons.Icons.ACCESS_TIME, label="Clock"),
        ],
